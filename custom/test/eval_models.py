...
def retrieve_and_rerank(
    ...
    # Extract chunk predictions for retrieval metrics
    preds = []
    for h in reranked:
        _, m = get_text_and_meta(h)
        preds.append((m.get("source_file"), m.get("chunk_index")))

    return reranked, rewritten, preds
...
def run_unified_evaluation(
    ...
    Returns: (retrieval_metrics, extraction_metrics)
    """
    ...
    retrieval_results = []
    # Cache: query_text -> (reranked_chunks, rewritten_query)
    chunk_cache: Dict[str, Tuple[List, str]] = {}

    total_queries = len(chunk_data)
    skipped = 0
    ...
    for i, item in enumerate(chunk_data, 1):
        ...
        try:
            ...
            # Cache for extraction phase
            chunk_cache[query] = (reranked, rewritten)
        except Exception as e:
            print(f"  ERROR: {e}")
            retrieval_results.append({"hit@k": 0, "rr": 0.0})
            time.sleep(2)
            continue
    ...
    print(f"\nRetrieval: Hit Rate={hit_rate:.3f}, MRR={mrr:.3f}, Recall@{TOP_K}={recall_at_k:.3f} (skipped {skipped})")
    ...
    print(f"Cached chunks available: {len(chunk_cache)}")
    print(f"{'='*60}\n")
    ...
    for i, item in enumerate(span_data, 1):
        ...
        try:
            # Check cache first (fuzzy match: check if query starts with same text)
